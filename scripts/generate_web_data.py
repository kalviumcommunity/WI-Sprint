...
OUT = Path(__file__).resolve().parent.parent / "web" / "assets" / "data" / "seed-data.js"
...
# 24 weeks so any trailing-12 view has a full prior 12 to compare against;
# the UI only ever charts the most recent 12.
WEEKS = trailing_weeks(24)
...
def main():
    ...
    # Emitted as a plain <script>-loadable assignment rather than .json so the
    # site works when opened straight off disk (file://), where fetch() of a
    # sibling file is blocked as cross-origin.
    body = json.dumps(payload, indent=None, separators=(",", ":"))
    OUT.write_text(
        "/* Generated by scripts/generate_web_data.py - do not edit by hand. */\n"
        f"window.IQ_SEED = {body};\n",
        encoding="utf-8",
    )
    print(
